...
class Car:
    ...
    def __init__(self):
        ...
        self.points_rot = [[0,0],[0,0],[0,0],[0,0]]
        self.Bat_rad = 1000
        self.Bat_count = 5
...
def draw_stats(screen, past_averages, past_maxes, network, layers, layer_outs, first_pass):
    if first_pass:
...
def main(ais, cars, past_averages, past_maxes, layers):
    frames = 0
    Tframes = 1800
    boost = False
    best_only = False
    ...
    while run:
        ...
        best_score = -100
        best_layer_outs = []
        for index, [x, car] in enumerate(cars):
            car.box_build()

            car.check_point(Goals)
            ais[x].fitness = car.score

            output = ais[x].gen_outs([car.echo(0), car.echo(1), car.echo(2), car.echo(3), car.echo(4), (car.x_speed**2 + car.y_speed**2)**0.5/5])
            ...
            if not no_display and not best_only:
                car.draw(screen)
            ...
            if ais[x].fitness > best_score:
                ...
                best_car = car
                best_ai = ais[x]
                best_layer_outs = best_ai.gen_layer_outs([car.echo(0), car.echo(1), car.echo(2), car.echo(3), car.echo(4), (car.x_speed**2 + car.y_speed**2)**0.5/5])

        if not no_display:
            if best_only:
                best_car.draw(screen)
            score_display(best_car.score, frames)
            draw_stats(screen, past_averages, past_maxes, best_ai.brain["network"], layers, best_layer_outs, frames == 1)
...
def run():
    ...
    random_ais_per_gen = 10
    num_of_inputs = 6
    hidden_nums = []
    num_of_outs = 2
    init_bais_range = [-5, 5]
    initial_ais = generate_initial_ais(num_of_cars, num_of_inputs, hidden_nums, num_of_outs, init_bais_range)
    ais = initial_ais
    past_averages = [0]
    past_maxes = [0]
    ...
    for i in range(10000):
        cars = [[j, Car()] for j in range(num_of_cars)]
        ais = main(ais, cars, past_averages, past_maxes, layers)
        print("generation ", i+1)
        print([round(ai.fitness, 3) for ai in sorted(ais, key=get_fitness)])
        print()
